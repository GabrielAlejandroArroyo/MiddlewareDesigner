...
async def crear_usuario(usuario_data: UsuarioCreateDTO):
    if usuario_data.id and await get_usuario_by_id(usuario_data.id):
        raise HTTPException(status.HTTP_409_CONFLICT, detail="El ID de usuario ya existe")
    if await get_usuario_by_nombre_usuario(usuario_data.nombre_usuario):
        raise HTTPException(status.HTTP_409_CONFLICT, detail="El nombre de usuario ya existe")
...
@router.patch("/{usuario_id}/cambiar-password", response_model=UsuarioReadDTO)
async def cambiar_contraseña(usuario_id: str, body: CambiarPasswordRequest):
    """Cambia la contraseña del usuario. Requiere contraseña actual."""
    return await cambiar_password(usuario_id, body.password_actual, body.password_nueva)
